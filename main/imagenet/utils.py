...
def attack_random(model,X,y,epsilon,trim):
...
def attack_pgd_linf(model, X, y, epsilon, alpha, attack_iters, restarts,trim,label=False):
    max_loss = torch.zeros(y.shape[0]).cuda()
    max_delta = torch.zeros_like(X).cuda()
    for _ in range(restarts):
        delta = torch.zeros_like(X).uniform_(-epsilon, epsilon).cuda()
        ...
        for _ in range(attack_iters):
            ...
            delta.grad.zero_()
        if label:
            all_loss = F.cross_entropy(model(X+delta,y), y, reduction='none')
        else:
            all_loss = F.cross_entropy(model(X+delta), y, reduction='none')
        max_delta[all_loss >= max_loss] = delta.detach()[all_loss >= max_loss]
        max_loss = torch.max(max_loss, all_loss)
