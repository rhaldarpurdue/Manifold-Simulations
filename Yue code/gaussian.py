...
def pgd_robustness(model,attack='none',epsilon=0.3,LOSS='ce'):
    model.eval()
    test_loss = 0
    test_acc = 0
    test_n = 0
...
def train(lr_max,epochs,lr_type='flat',attack='none',epsilon=0.3,LOSS='ce'):
    ...
    model.second.requires_grad=False
    ...
    opt = torch.optim.SGD(filter(lambda p: p.requires_grad, model.parameters()), lr=lr_max, weight_decay=0.3)
    fname='manifold_model/circle_codim-'+str(codim)+'nclass-'+str(n_class)+'.pth'
    Loss,acc=pgd_robustness(model, attack='l2',LOSS='mse',epsilon=1)
    print(Loss,acc)
    ...
    for epoch in range(epochs):
        ...
        # print(model.first.weight.data[0,:])
        Loss,acc=pgd_robustness(model, attack='l2',LOSS='mse',epsilon=1)
...
for codim in [int(sys.argv[1])]:
    ...
    train_loader = torch.utils.data.DataLoader(train_data, batch_size=800, shuffle=False)
    
    
    #m=train(1e-3,50)
    m=train(1e-4,500,attack='l2',epsilon=0,LOSS='mse')
